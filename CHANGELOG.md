...
### Changed
- **The beachgoer folds down and rises smoothly.** Instead of snapping between
  standing and reclining, the woman now plays two in-between poses each way —
  a bowed **crouch** kneeling onto the towel, then a **propped** sit with her
  legs stretching out — before settling into the sunbathe, and mirrors them in
  reverse when she gets up. Her head steps down (and back up) a pixel at a time
  so the transition reads as one continuous motion.
...
### Changed
- **Beachgoer polish.** The woman/cat sit a pixel higher and the sunbather
  settles a couple pixels to the left so her towel no longer touches the sign.
  Her **sunbathe is now ~30 seconds** — then she gets up, walks on to the right
  behind the sign and out of frame, and the **next stroller arrives ~5–10s
  later**. The reclining pose is redrawn to read more clearly as a woman: head
  and flowing hair, bikini top and hip, and a raised bent knee.
